...
namespace zFramework.IO
{
    public static class FileDialog
    {
        ...
        public static List<string> SelectFile(string title,params string[] extensions)
        {
          var filters = new List<string>
            ...
            foreach (var ext in extensions)
            {
                ...
                {
                    Debug.Log($"{nameof(FileDialog)}: 文件扩展参数格式是： 描述+ 竖线+后缀名 ，例如：应用程序|exe");
                }
            }
        ...
        public static string SaveDialog(string title, string path,string extensionDesc)
        {
            ...
            OpenFileName ofn = new OpenFileName();
            ofn.lStructSize = Marshal.SizeOf(ofn);
            ofn.lpstrFilter = Filter("All Files", "*.*", string.IsNullOrEmpty(extensionDesc)?extension:extensionDesc, "*" + extension);
            ofn.nFilterIndex = 2;
